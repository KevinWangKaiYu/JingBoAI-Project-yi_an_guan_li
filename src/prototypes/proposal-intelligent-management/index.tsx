...
function DigitalEmployeeFlow({ notice }: { notice: (s: string) => void }) {
  ...
  useEffect(() => { if (!jumpCue) { setJumpFrame(null); return; } const timer = window.setTimeout(() => setJumpCue(null), 2500); return () => window.clearTimeout(timer); }, [jumpCue]);
  useLayoutEffect(() => {
    if (!jumpCue) return;
  ...
  return <div className="bp-lane" style={{ left, top, width: right - left, height: bottom - top }} key={`${stage.lane}-${index}`} aria-label={`${stage.agentTitle}流程分区`} />;
    ...
    {jumpFrame && <svg className="bp-jump-overlay" aria-hidden="true" key={`jump-frame-${jumpFrame.token}`}><defs><filter id="bp-moving-frame-glow"><feGaussianBlur stdDeviation="3" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>{[0, .98].map((begin) => <g key={begin}><rect className="bp-moving-frame" x={jumpFrame.from.x} y={jumpFrame.from.y} width={jumpFrame.from.width} height={jumpFrame.from.height} rx="10" filter="url(#bp-moving-frame-glow)"><animate attributeName="x" begin={`${begin}s`} dur=".76s" values={`${jumpFrame.from.x};${jumpFrame.to.x}`} fill="freeze"/><animate attributeName="y" begin={`${begin}s`} dur=".76s" values={`${jumpFrame.from.y};${jumpFrame.to.y}`} fill="freeze"/><animate attributeName="width" begin={`${begin}s`} dur=".76s" values={`${jumpFrame.from.width};${jumpFrame.to.width}`} fill="freeze"/><animate attributeName="height" begin={`${begin}s`} dur=".76s" values={`${jumpFrame.from.height};${jumpFrame.to.height}`} fill="freeze"/><animate attributeName="opacity" begin={`${begin}s`} dur=".76s" values="0;1;1;0" fill="freeze"/></rect></g>)}</svg>}    {selectedNode && <aside className="bp-task-drawer" aria-label={`${selectedNode.title}模块子任务`}>
