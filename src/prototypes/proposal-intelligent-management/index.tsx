...
function DigitalEmployeeFlow({ notice, items }: { notice: (s: string) => void; items: Proposal[] }) {
  ...
  const [jumpCue, setJumpCue] = useState<{ fromId: string | null; fromSubtaskKey?: string; toId: string; subtaskKey?: string; fromRect: { x: number; y: number; width: number; height: number }; token: number } | null>(null);
  ...
  useEffect(() => { if (!jumpCue) { setJumpFrame(null); return; } const timer = window.setTimeout(() => setJumpCue(null), 3000); return () => window.clearTimeout(timer); }, [jumpCue]);
  useLayoutEffect(() => {
    if (!jumpCue) return;
    const target = jumpCue.subtaskKey ? subtaskElementRefs.current.get(jumpCue.subtaskKey) : nodeElementRefs.current.get(jumpCue.toId);
    if (!target) return;
    const rect = target.getBoundingClientRect();
    setJumpFrame({ from: jumpCue.fromRect, to: { x: rect.x, y: rect.y, width: rect.width, height: rect.height }, token: jumpCue.token });
  }, [jumpCue, selectedNode, selectedSubtask]);
  useEffect(() => {
    const fit = () => {
  ...
  const nodeData = selectedTraceNode ? (() => {
    const isModificationSummary = selectedTraceNode.includes("修改信息识别 · 修改总结");
    if (isModificationSummary) {
      return {
        normal: [
  ...
  const nodeDataModal = selectedTraceNode && nodeData && <div className="bp-node-data-modal-backdrop" onClick={() => setSelectedTraceNode(null)}><section className={`bp-node-data-modal ${selectedTraceNode.includes("修改信息识别 · 修改总结") ? "bp-modification-summary-data" : ""}`} role="dialog" aria-modal="true" aria-label={`${selectedTraceNode} 节点详情`} onClick={(event) => event.stopPropagation()}><header><button type="button" className="bp-data-back" onClick={() => setSelectedTraceNode(null)} aria-label="返回二级页面"><ChevronRight size={16}/>返回</button><div><small>NODE BUSINESS DETAIL</small><h3>{selectedTraceNode}</h3><p>{selectedTraceNode.includes("修改信息识别 · 修改总结") ? "展示本次修改信息比对与汇总所依据的议案变更内容。" : "本节点仅展示发生变更的业务字段。"}</p></div></header><section><h4>{selectedTraceNode.includes("修改信息识别 · 修改总结") ? "基本信息" : "议案基础信息"}</h4>{nodeData.normal.map((record) => renderNodeRecord(record))}</section>{nodeData.changed.length > 0 && <section className="bp-changed-data"><h4>{selectedTraceNode.includes("修改信息识别 · 修改总结") ? "议案修改内容" : "本环节更新字段"}</h4>{nodeData.changed.map((record) => renderNodeRecord(record, true))}</section>}<footer><div><span>调用 Skill</span>{nodeData.skills.length ? nodeData.skills.map((skill) => <b key={skill}>{skill}</b>) : <b>本节点未调用 Skill</b>}</div><div><span>调用 MCP</span>{nodeData.mcps.length ? nodeData.mcps.map((mcp) => <b key={mcp}>{mcp}</b>) : <b>本节点未调用 MCP</b>}</div></footer></section></div>;
